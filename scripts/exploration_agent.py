...
"""Run exploration rollouts with random, demo, or greedy controllers."""
...
parser = argparse.ArgumentParser(description="Exploration rollout agent for Isaac Lab environments.")
parser.add_argument(
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Use the greedy exploration controller instead of random actions.",
)
...
def main():
    """Run an exploration rollout with the requested controller."""
    ...
    greedy_agent = None
    if args_cli.greedy:
        greedy_agent = RewardGuidedExplorer(
            env=env,
            car_names=car_names,
        ...
        print("[INFO]: Using greedy exploration controller instead of random actions.")
    ...
    if greedy_agent is None:
        ...
        print("[INFO]: Recomputing greedy exploration actions every environment step.")
        ...
        while simulation_app.is_running() and (args_cli.max_steps is None or step_count < args_cli.max_steps):
            ...
            with torch.inference_mode():
                if greedy_agent is not None:
                    actions = greedy_agent.compute_actions(done)
